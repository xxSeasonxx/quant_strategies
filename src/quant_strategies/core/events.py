...
class StageEmitter:
    """Structured stage-event emitter shared by the runner and validation.

    Subclasses set ``event_type`` to namespace their events (``runner_stage`` vs
    ``validation_stage``); the emit/stage logic lives here once.
    """
...
def jsonl_event_sink(stream: TextIO) -> StageEventSink:
