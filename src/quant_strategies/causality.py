...
def _row_visible_for_decision(row: Mapping[str, Any], decision: StrategyDecision) -> bool:
    ...
    if available_value is not None:
        ...
        # Availability parse failures are evidence-quality problems. Replay
        # falls back to timestamp-only visibility so bad provenance does not
        # masquerade as a hidden-lookahead strategy failure.
        return True
