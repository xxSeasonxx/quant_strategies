...
`P0` = close before trusting an automated climb. `P1` = important for live feasibility. `P2` = hygiene/clarity. Status starts `Open`.

> **Sequencing (revised after the independent audit — §1A "Audit" note):** **0a** (accounting spine + fail-closed verdict — *surgical, no strategy/contract rewrites*) is the true first move; it absorbs No. 1/5/16 and the F7 cluster and closes fail-open. Then **No. 2** (honest at-risk statistic + min-sample gate) and **No. 3** (cost floor + capacity) — both independent of the ontology. Then **0b** (construction layer + operator-frozen budget). Then **0c** (target-book ontology cutover — the *only* repo-wide breaking change), after the score is already honest. Independent anytime: **No. 11** (PIT guard), **No. 17** (artifacts), dead-code (No. 12). This **supersedes** the earlier "fix the root, break everything, rerun": the *accounting* root is surgical; only the *ontology* root breaks strategies.
