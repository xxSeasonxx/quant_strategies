...
negative enough and price has extended down. It keeps ADA only when either price
extension or funding pressure is strong, allows BTC only on severe extension,
uses a 50-minute cadence, and keeps the successful 0.3 target weight from the
best non-BTC survivor. It uses an 8-hour max hold and requires stronger summed
negative funding pressure to test whether cleaner crowding states improve
robustness.
...
_ADA_MIN_LONG_RETURN_EXTENSION_BPS = 10.0
_ADA_STRONG_NEGATIVE_FUNDING_BPS = 2.0
_ADA_LATEST_NEGATIVE_FUNDING_BPS = 1.0
_ADA_COVERAGE_MIN_FUNDING_PRESSURE_BPS = 1.0
_BTC_MIN_LONG_RETURN_EXTENSION_BPS = 75.0
_LONG_MIN_FUNDING_PRESSURE_BPS = 1.5
...
def _generate_signal_payloads(
    ...
    for as_of_time in as_of_times:
        ...
        negative_tail = [
            candidate
            for candidate in candidates
            if _passes_long_funding_pressure(candidate, max(min_abs_funding_bps, _LONG_MIN_FUNDING_PRESSURE_BPS))
            and candidate["return_extension_bps"] <= -min_abs_return_bps
            and _passes_symbol_long_extension(candidate)
            and abs(candidate["latest_funding_bps"]) >= min_latest_abs_funding_bps
            and _passes_return_z(candidate, min_abs_return_z)
...
def _passes_long_funding_pressure(
    ...
    threshold = min_abs_funding_bps
    if (
        candidate["symbol"] == "ADA-PERP"
        and float(candidate["latest_funding_bps"]) <= -_ADA_LATEST_NEGATIVE_FUNDING_BPS
    ):
        threshold = min(threshold, _ADA_COVERAGE_MIN_FUNDING_PRESSURE_BPS)
    return float(candidate["funding_pressure_bps"]) <= -threshold


def _passes_symbol_long_extension(candidate: Mapping[str, Any]) -> bool:
    if candidate["symbol"] != "ADA-PERP":
        if candidate["symbol"] != "BTC-PERP":
            return True
        return float(candidate["return_extension_bps"]) <= -_BTC_MIN_LONG_RETURN_EXTENSION_BPS
    return (
        float(candidate["return_extension_bps"]) <= -_ADA_MIN_LONG_RETURN_EXTENSION_BPS
        or float(candidate["funding_pressure_bps"]) <= -_ADA_STRONG_NEGATIVE_FUNDING_BPS
        or float(candidate["latest_funding_bps"]) <= -_ADA_LATEST_NEGATIVE_FUNDING_BPS
    )
