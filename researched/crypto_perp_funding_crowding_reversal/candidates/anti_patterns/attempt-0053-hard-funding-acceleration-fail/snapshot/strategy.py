...
higher-edge non-BTC long book, uses a 100-minute cadence, and requires the
latest funding print to be at least as negative as the recent average. It uses an
8-hour max hold to test whether the reversal edge decays before the current
12-hour long horizon.
...
def _generate_signal_payloads(
    ...
    for as_of_time in as_of_times:
        ...
        negative_tail = [
            ...
            and _passes_funding_acceleration(candidate, funding_lookback_events)
            and candidate["return_extension_bps"] <= -min_abs_return_bps
            and abs(candidate["latest_funding_bps"]) >= min_latest_abs_funding_bps
            and _passes_return_z(candidate, min_abs_return_z)
...
def _passes_funding_acceleration(candidate: Mapping[str, Any], funding_lookback_events: int) -> bool:
    average_funding_bps = float(candidate["funding_pressure_bps"]) / funding_lookback_events
    return float(candidate["latest_funding_bps"]) <= average_funding_bps
