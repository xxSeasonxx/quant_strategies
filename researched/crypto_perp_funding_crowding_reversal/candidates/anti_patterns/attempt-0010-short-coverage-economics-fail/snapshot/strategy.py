...
variant keeps repeated long tranches for persistent negative-funding
dislocations and restores the plain positive-funding short book as a symmetric
coverage leg, without anchor-specific filters or special fast short exits.
...
def _generate_signal_payloads(
    ...
    for as_of_time in as_of_times:
        ...
        negative_tail = [
            candidate
            for candidate in candidates
            if _passes_long_funding_pressure(candidate, min_abs_funding_bps, min_same_sign_funding_events)
            and candidate["return_extension_bps"] <= -min_abs_return_bps
            and abs(candidate["latest_funding_bps"]) >= min_latest_abs_funding_bps
            and _passes_return_z(candidate, min_abs_return_z)
...
def _passes_long_funding_pressure(
    ...
    min_same_sign_funding_events: int,
) -> bool:
    return (
        float(candidate["funding_pressure_bps"]) <= -min_abs_funding_bps
        and int(candidate["funding_same_sign_events"]) >= max(2, min_same_sign_funding_events - 1)
    )
