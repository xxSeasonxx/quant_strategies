...
higher-edge non-BTC long book, uses a 100-minute cadence, and trades only the
three most negative-funding dislocations at each decision time. It caps repeated
active tranches per symbol to avoid overstacking one persistent dislocation. It
uses an 8-hour max hold to test whether the reversal edge decays before the
current 12-hour long horizon.
...
def _generate_signal_payloads(
    ...
    long_signal_times_by_symbol: dict[str, list[datetime]] = {}
    for as_of_time in as_of_times:
        ...
        negative_tail = [
            candidate
            for candidate in candidates
            if _passes_long_funding_pressure(candidate, min_abs_funding_bps)
            and candidate["return_extension_bps"] <= -min_abs_return_bps
            and abs(candidate["latest_funding_bps"]) >= min_latest_abs_funding_bps
            and _passes_return_z(candidate, min_abs_return_z)
        ...
        selected_longs = (
            _selected_tail(negative_tail, "long", selection_score, min(top_n, 3))
        ...
        if include_negative_funding_longs:
            for candidate in selected_longs:
                ...
                if _passes_symbol_cooldown(
                    ...
                    last_signal_time_by_symbol,
                    symbol_cooldown_minutes,
                ) and _passes_long_tranche_cap(
                    candidate["symbol"],
                    decision_time,
                    long_signal_times_by_symbol,
                    max_tranches=2,
                    window_minutes=short_hold_bars,
                ):
                    signals.append(
                        _signal(
                            candidate,
                    ...
                    last_signal_time_by_symbol[candidate["symbol"]] = decision_time
                    _record_long_tranche(candidate["symbol"], decision_time, long_signal_times_by_symbol)
                    _record_active_window(
                        candidate["symbol"],
                        decision_time,
                        hold,
...
def _passes_long_tranche_cap(
    symbol: str,
    decision_time: datetime,
    signal_times_by_symbol: dict[str, list[datetime]],
    *,
    max_tranches: int,
    window_minutes: int,
) -> bool:
    cutoff = decision_time - timedelta(minutes=window_minutes)
    recent = [timestamp for timestamp in signal_times_by_symbol.get(symbol, []) if timestamp > cutoff]
    signal_times_by_symbol[symbol] = recent
    return len(recent) < max_tranches


def _record_long_tranche(
    symbol: str,
    decision_time: datetime,
    signal_times_by_symbol: dict[str, list[datetime]],
) -> None:
    signal_times_by_symbol.setdefault(symbol, []).append(decision_time)
