...
negative enough and price has extended down. It evaluates that long-only
dislocation every 90 minutes inside the configured sparse cadence so persistent
dislocations can produce enough observations in otherwise sparse Train regimes.
It uses an 8-hour max hold to test whether the reversal edge decays before the
current 12-hour long horizon. A trailing stop tests whether favorable reversals
should be protected after they start without imposing a fixed take-profit cap.
...
def _generate_signal_payloads(
    ...
    exit_controls.setdefault("trailing_stop_bps", 120.0)
...
def _is_long_decision_time(timestamp: datetime, decision_interval_minutes: int, params: Mapping[str, object]) -> bool:
    ...
    if not session_start_hour <= timestamp.hour < session_end_hour:
        return False
    minute_of_day = timestamp.hour * 60 + timestamp.minute
    interval = (decision_interval_minutes * 3) // 8
