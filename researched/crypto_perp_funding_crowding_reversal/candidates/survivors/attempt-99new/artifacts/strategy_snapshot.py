...
def generate_decisions(
    ...
    for symbol in sorted(by_symbol):
        ...
        for index, signal in enumerate(symbol_signals):
            ...
            target = float(signal["weight"]) if side == "long" else -float(signal["weight"])
...
def _risk_rule(signal: Mapping[str, object]) -> RiskRule | None:
    ...
    if "stop_loss_bps" in signal:
        legs["stop_loss"] = (
            _optional_positive_float(signal.get("stop_loss_bps"), "stop_loss_bps") / 1e4
        )
    if "take_profit_bps" in signal:
        legs["take_profit"] = (
            _optional_positive_float(signal.get("take_profit_bps"), "take_profit_bps") / 1e4
        )
    if "trailing_stop_bps" in signal:
        legs["trailing"] = (
            _optional_positive_float(signal.get("trailing_stop_bps"), "trailing_stop_bps") / 1e4
        )
    if not legs:
        return None
        ...
        candidates.append(
            {
                ...
                "observation_points": (
                    ...
                    *(
                        ...
                        for timestamp in funding_stats["funding_observation_timestamps"]
                    ),
                ),
            }
        )
