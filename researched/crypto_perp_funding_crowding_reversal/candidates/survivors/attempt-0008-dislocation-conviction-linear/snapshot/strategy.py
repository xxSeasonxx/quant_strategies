...
def _select_candidates(
    ...
    # cross-section mean. In "raw" mode it is bps; in "vol_normalized" mode each
    # name's extension is divided by its expected move over the lookback horizon
    # (per-minute vol scaled by sqrt(horizon)), so the relative-value screen is
    # comparable across names of very different volatility.
    ...
    for candidate in candidates:
        ...
        if dislocation is None:
            continue
        idio_long = market_dislocation - dislocation
        idio_short = dislocation - market_dislocation
        if (
...
def _selection_targets(
    ...
    if weighting == "dislocation":
        weights = [max(0.0, selection.idiosyncratic) for selection in selections]
...
def _idiosyncratic_mode(value: object) -> str:
    parsed = str(value)
    if parsed not in {"raw", "vol_normalized"}:
        raise ValueError("idiosyncratic_mode must be one of: raw, vol_normalized")
    return parsed
