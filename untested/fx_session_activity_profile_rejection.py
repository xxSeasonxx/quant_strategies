...
def generate_decisions(
    ...
    for symbol, symbol_rows in sorted(rows_by_symbol.items()):
        for index, row in enumerate(symbol_rows):
            ...
            profile_rows = _profile_rows(symbol_rows, timestamp, parsed)
...
def _acceptance_decisions(
    ...
    activity = _activity_signal(rows, crossing_row, params)
    if activity is None or activity["zscore"] < float(params["min_activity_z"]):
        return []
    if not _spread_allowed(rows, current, params):
        return []
    ...
    return [
        _decision(
            ...
            profile_rows,
            sequence,
            _decision_context_rows(rows, current, params, activity["baseline_rows"]),
            profile,
            activity["zscore"],
            rule,
            params,
...
def _rejection_decisions(
    ...
    activity = _activity_signal(rows, current, params, baseline_end=_timestamp(latest_breach))
    if activity is None or activity["zscore"] < float(params["min_activity_z"]):
        return []
    if not _spread_allowed(rows, current, params):
        return []
